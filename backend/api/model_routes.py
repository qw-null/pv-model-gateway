...
# model_routes.py - 修改 _model_to_dict，增加 inputs/outputs

def _model_to_dict(m: ModelRecord, related: dict = None) -> dict:
    meta = m.meta_json or {}          # ← 新增：从 meta_json 取 inputs/outputs
    return {
        "id":          m.id,
        "name":        m.name,
        "title":       m.title,
        ...
        "tags":        [],
        "inputs":      meta.get("inputs",  []),   # ← 新增
        "outputs":     meta.get("outputs", []),   # ← 新增
...
def get_all_model_names(db: Session = Depends(get_db)):
    ...
    return {
        "success": True,
        "data": [
            {"name": r.name, "title": r.title, "category": r.category}
            for r in records
        ]
    }
...
def get_model(name: str, db: Session = Depends(get_db)):
    """
    前端调用：getModelDetail(name) → GET /api/models/{name}
    """
    record = db.query(ModelRecord).filter(
        ModelRecord.name == name,
        ModelRecord.is_active == True
