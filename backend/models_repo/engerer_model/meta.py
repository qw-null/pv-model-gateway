MODEL_META = {
    ...
    "version": "1.0.0",
    "description": "Engerer 提出的三种分离模型：Engerer1（非云增强条件）、Engerer2（全天空条件，增加云增强线性校正项）、Engerer3（晴天条件优化系数）。基于 S 型逻辑函数，引入视太阳时 AST、天顶角 Z 和晴空偏差 Δktc。",
    "author": "PV Team",
    "category": "辐照分离",
    "related_models": {
        "pre": ["solar_position"],
    ...
    "inputs": [
        ...
        },
        {
            ...
            "description": "模型类型选择：engerer1（非云增强）/ engerer2（全天空）/ engerer3（晴天优化）"
        }
    ],
    "outputs": [
        {
        ...
        },
        {
            ...
            "type": "float",
            "unit": "",
            "description": "云增强漫射比例 kde（仅 Engerer2 有效，其余模型返回 0.0）"
        }
    ],
    "tags": ["engerer", "irradiance", "separation"],
    "execution": {
        "timeout": 10,
        "cacheable": True,
        "cache_ttl": 1800
