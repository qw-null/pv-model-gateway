      ...
      <el-tab-pane label="1.基本数据" name="basic">
        <el-form :model="form" label-width="140px" size="default">
          ...
          <el-row :gutter="24" class="readonly-row">
            ...
            <el-col :span="12">
              <div class="calc-item">短路电流温度系数 muISC：<span>{{ form.temp_coeff }} mA/℃</span></div>
            </el-col>
            <el-col :span="12">
              <div class="calc-item">开路电压温度系数 muVocSpec：<span>{{ form.mu_voc_spec }} mV/℃</span></div>
            </el-col>
            <el-col :span="12">
              <div class="calc-item">功率温度系数 muPmpReq：<span>{{ form.mu_pmp }} %/℃</span></div>
            </el-col>
          </el-row>
      ...
      <el-tab-pane label="2.IAM数据" name="iam">
        <div class="iam-table">
          <div class="iam-row header">
            ...
            <div v-for="(angle, idx) in form.iam_angles" :key="idx" class="iam-cell">
              <el-input v-model.number="form.iam_angles[idx]" size="small" />
            </div>
          </div>
          <div class="iam-row">
            <div class="iam-cell label">IAM</div>
            <div v-for="(val, idx) in form.iam_values" :key="idx" class="iam-cell">
        ...
        <el-form label-width="100px" size="default" class="curve-form">
          ...
          <template v-if="curveParam === 'irradiance'">
            ...
            <el-form-item label="辐照度 W/m²">
              <div class="param-inputs">
                <el-input v-for="(_, idx) in irradiances" :key="idx" v-model.number="irradiances[idx]"
                  style="width:90px" />
              </div>
            </el-form-item>
          </template>
          ...
          <template v-else>
            ...
            <el-form-item label="温度 ℃">
              <div class="param-inputs">
                <el-input v-for="(_, idx) in temperatures" :key="idx" v-model.number="temperatures[idx]"
                  style="width:90px" />
              </div>
            </el-form-item>
          </template>
...
function renderIamChart() {
  ...
  chartInstance.setOption({
    ...
    tooltip: { trigger: 'axis' ,formatter: '入射角：{b}°<br/>IAM系数：{c}'},
    xAxis: {
      type: 'category',
      name: '入射角（度）',
  ...
  curveChart.setOption({
    ...
    tooltip: {
      ...
      formatter(params) {
        ...
        let html = `电压: ${v} V<br/>`
        params.forEach(p => {
          html += ` ${isIV ? '电流' : '功率'}: ${p.data[1]?.toFixed(2)} ${unit}<br/>`
        })
        return html
      },
    },
