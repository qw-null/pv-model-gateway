    ...
    <el-page-header @back="$router.push('/')" style="margin-bottom:24px;">
      <template #content>
        <span style="font-size:18px; font-weight:700;">
          {{ isEdit ? `编辑模型：${route.params.name}` : '新增模型' }}
        </span>
      </template>
    </el-page-header>
      ...
      <el-col :span="8">
        ...
        <el-card header="操作" style="margin-bottom:16px;">
          ...
          <div style="font-size:12px; color:#94a3b8; line-height:2;">
            <div style="display:flex; align-items:center; gap:6px;">
              ...
              <span>meta.py — {{ validatePassed ? '校验通过' : '待校验' }}</span>
            </div>
            <div style="display:flex; align-items:center; gap:6px;">
        ...
        <el-card header="校验结果" style="margin-bottom:16px;" v-if="validateResult">
          <el-result
            ...
            :title="validateResult.valid ? '校验通过，可以发布' : '校验失败，请修正后重试'"
            style="padding:12px 0;"
          >
            <template #extra>
              <!-- 错误列表 -->
              <div v-if="validateResult.errors && validateResult.errors.length">
                <el-tag
                  v-for="err in validateResult.errors"
                  :key="err"
                  type="danger"
                  style="display:block; margin-bottom:6px;
                         white-space:normal; height:auto; line-height:1.6;"
                >
                  {{ err }}
                </el-tag>
              </div>
              <!-- 警告列表 -->
              <div
                v-if="validateResult.warnings && validateResult.warnings.length"
                style="margin-top:8px;"
              >
                <el-tag
                  v-for="w in validateResult.warnings"
                  :key="w"
                  type="warning"
                  style="display:block; margin-bottom:6px;
                         white-space:normal; height:auto; line-height:1.6;"
...
onMounted(async () => {
  if (isEdit.value) {
    try {
      ...
      metaCode.value  = res.data.meta_code  || ''
      modelCode.value = res.data.model_code || ''
      // 编辑模式下已有代码，尝试解析预览
...
async function handleValidate() {
  ...
  try {
    ...
    if (res.data.valid) {
      ...
      ElMessage.error(`校验失败：${res.data.errors?.[0] || '请查看详细错误'}`)
    }
  } catch (e) {
...
async function handleSave() {
  ...
  try {
    ...
    // 区分 meta.py 和 model.py 的校验失败
    const detail = e.response?.data?.detail
    if (detail?.message === 'model.py 校验失败') {
      ElMessage.error(`model.py 校验失败：${detail.errors?.[0]}`)
      activeTab.value = 'model'
    } else if (detail?.message === 'meta.py 校验失败') {
      ElMessage.error(`meta.py 校验失败：${detail.errors?.[0]}`)
